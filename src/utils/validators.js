...
const maxLength = (max) => (value) => (value && value.length > max ? `Must be ${max} or less characters` : undefined)
const minLength = (min) => (value) => (value && value.length < min ? `Must be ${min} or more characters` : undefined)
const number = (value) => (value && Number.isNaN(Number(value)) ? 'Must be a number' : undefined)
const minValue = (min) => (value) => (value && value < min ? `Must be at least ${min}` : undefined)
const maxValue = (max) => (value) => (value && value > max ? `Must not be more than ${max}` : undefined)
...
const alphaNumeric = (value) => (value && /[^a-zA-Z0-9 ]/i.test(value)
  ...
  : undefined)
const folderName = (value) => (value && /[^a-zA-Z0-9-]/i.test(value)
  ? 'Only alphanumeric characters - no spaces'
  : undefined)
const phone = (value) => (value && !/^(0|[1-9][0-9]{9})$/i.test(value)
  ? 'Invalid phone number, must be 10 digits'
  : undefined)
const numeric = (value) => (Number.isNaN(parseFloat(value)) ? 'Must be a numeric value' : undefined)
